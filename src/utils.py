...
def normalize_datatype(data_type):
    normalized_datatype=re.sub(r'\b\d+ x\b', 'NUM x', data_type)
...
#    model.wv.save_word2vec_format(corpus_vec_path)
    ...
    for homoG_file in tqdm(homoG_files,total=2385, desc="initialing node feature:"):
        homoG=torch.load(homoG_file)
        programl_file_name = os.path.basename(homoG_file).replace('.pth', '.prog')
        programl_file_path=os.path.join(ir_programl_dir,programl_file_name)
