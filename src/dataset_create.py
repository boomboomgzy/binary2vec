...
import itertools
from utils import remove_metadata_comma_align,ir_validation,inst_node_isvalid,collect_homoG_files,normalize_inst,normalize_datatype
        ...
        # (control,input,output,call) control:inst->inst  input:data->inst output:inst->data   call:inst->inst
            ...
            for global_idx,node in enumerate(graph.node):
                if node.type==0:
                    ...
                    node.text=normalize_datatype(node.text)
...
if __name__=='__main__':
    ...
    homoG_save_dir=os.path.join(save_dir,'homoG')
    prep_log_dir=os.path.join(save_dir,'invalid_IR')
    prep_save_dir=os.path.join(save_dir,'preprocessed_IR')
    ir_programl_dir=os.path.join(save_dir,'ir_programl')
    homoG_dataset_dir=os.path.join(root,'homoG_dataset')
    edge_type_file_path=os.path.join(save_dir,'homoG_edge_type.json')
    ...
    if edge_type_file_path!=None:
        with open(edge_type_file_path,'r') as file:
            homoG_edge_type_dict=json.load(file)

    homoG_edge_type_count=len(homoG_edge_type_dict.keys())
